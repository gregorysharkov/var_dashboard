...
from helper import historical_series, imply_SMB_GMV
import numpy as np
import pandas as pd
...
def FactorBetas(factor_prices: pd.DataFrame, position_prices: pd.DataFrame):
    ...
    factor_returns = (factor_prices.iloc[1:,] / factor_prices.iloc[1:,].shift(1)) - 1
    ...
    factor_returns_rf = factor_returns.iloc[:, 1:] - factor_rf_ret_mat.iloc[:, :]
    factor_rf_ret_mat_for_pos = pd.DataFrame(
        np.tile(
            factor_returns.iloc[:, 0].values[:, None], len(position_returns.columns)
        ),
        index=factor_returns.index,
    ...
    for i in range(0, len(factor_returns_rf.columns)):
        ...
        beta_factors_df_by_factor = pd.DataFrame(beta_factors_df_list_by_factor,
                                                 columns=[factor_returns_rf.columns[i]],
                                                 index=np.repeat(factor_prices.index[-1], len(position_prices.columns)))
        beta_factors_df_by_factor = pd.concat([position_prices_df, beta_factors_df_by_factor], axis=1)
    ...
    beta_factors_df_list_across_factors = beta_factors_df_list_across_factors.loc[:, ~beta_factors_df_list_across_factors.columns.duplicated()]
