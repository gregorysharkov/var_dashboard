...
def get_market_trading_days(start_date: str, end_date: str) -> pd.DatetimeIndex:
    ...
    market_trading_days_range = nyse.schedule(start_date=start_date, end_date=end_date)
...
if __name__ == "__main__":
    ...
    position['Quantity']=position['Quantity'].apply(lambda x: x.replace(",",""))
    position['Quantity']=position['Quantity'].apply(lambda x: x.replace("$",""))
    position['Quantity']=position['Quantity'].apply(lambda x: x.replace("(","-"))
    position['Quantity']=position['Quantity'].apply(lambda x: x.replace(")",""))
    ...
    market_trading_days_range = get_market_trading_days(price_date_min, price_date_max)
    market_trading_days_range = market_trading_days_range.index.strftime("%Y-%m-%d")
    ...
    factor_ids = [item for item in factor_ids_full if item not in factors_to_remove]
    ...
    from reportwriter import obj_reportwriter
    writer = pd.ExcelWriter(f"output/risk_report_{now}.xlsx", engine="xlsxwriter")
    rw = obj_reportwriter(writer)
...
# Load the dataframe using pickle
    ...
    if saved_VaR:     
        matrix_correlation = pd.read_pickle('matrix_correlation.pkl')
        VaR_Top10 = pd.read_pickle('VaR_Top10.pkl')
        VaR_Bottom10= pd.read_pickle('VaR_VaR_Bottom10Top10.pkl')
        VaR_structured_strat= pd.read_pickle('VaR_structured_strat.pkl')
        VaR_structured_sector= pd.read_pickle('VaR_structured_sector.pkl')
        VaR_structured_industry= pd.read_pickle('VaR_structured_industry.pkl')
        VaR_structured_country= pd.read_pickle('VaR_structured_country.pkl')
        VaR_structured_mcap= pd.read_pickle('VaR_structured_mcap.pkl')
        matrix_cov= pd.read_pickle('matrix_cov.pkl')
        decay_cov= pd.read_pickle('decay_cov.pkl')        
    else:
        ...
        VaR95_top_10, VaR95_bottom_10 = VaR.filter_VaR95(
            factor_prices, position, factor_betas, matrix_cov
        )
        VaR99_top_10, VaR99_bottom_10 = VaR.filter_VaR99(
            factor_prices, position, factor_betas, matrix_cov
        )
        VaR95_filtered_iso = VaR.filter_VaR95_iso(
            filters_dict, factor_prices, position, factor_betas, matrix_cov
        )
        VaR_Top10 = pd.merge(VaR95_top_10, VaR99_top_10, left_index=True, right_index=True)
        ...
        matrix_correlation.to_pickle('matrix_correlation.pkl')
        VaR_Top10.to_pickle('VaR_Top10.pkl')
        VaR_Bottom10.to_pickle('VaR_VaR_Bottom10Top10.pkl')
        VaR_structured_strat.to_pickle('VaR_structured_strat.pkl')
        VaR_structured_sector.to_pickle('VaR_structured_sector.pkl')
        VaR_structured_industry.to_pickle('VaR_structured_industry.pkl')
        VaR_structured_country.to_pickle('VaR_structured_country.pkl')
        VaR_structured_mcap.to_pickle('VaR_structured_mcap.pkl')
        matrix_cov.to_pickle('matrix_cov.pkl')
        decay_cov.to_pickle('decay_cov.pkl')
       
        
    rw.write_FactorCorrels("FactorCorrels", matrix_correlation.reset_index())
    rw.write_varReport("VaRReport", VaR_Top10.reset_index(), VaR_Bottom10.reset_index(), VaR_structured_strat.reset_index(), VaR_structured_sector.reset_index(),VaR_structured_industry.reset_index(),VaR_structured_country.reset_index(), VaR_structured_mcap.reset_index())
    
    #rw.close_writer()
    
    if saved_stress:     
        stress_test_beta_price_vol_results_df= pd.read_pickle('stress_test_beta_price_vol_results_df.pkl')
        stress_test_price_vol_results_df= pd.read_pickle('stress_test_price_vol_results_df.pkl')
        stress_test_price_vol_exposure_results_df= pd.read_pickle('stress_test_price_vol_exposure_results_df.pkl')
        options_delta_adj_exposure_calc= pd.read_pickle('options_delta_adj_exposure_calc.pkl')
        options_delta1_exposure_calc= pd.read_pickle('options_delta1_exposure_calc.pkl')
        greek_sensitivities_calc= pd.read_pickle('greek_sensitivities_calc.pkl')
        options_premium_calc= pd.read_pickle('options_premium_calc.pkl')
    else:
        ...
        stress_test_beta_price_vol_results_df.to_pickle('stress_test_beta_price_vol_results_df.pkl')
        stress_test_price_vol_results_df.to_pickle('stress_test_price_vol_results_df.pkl')
        stress_test_price_vol_exposure_results_df.to_pickle('stress_test_price_vol_exposure_results_df.pkl')
    ...
    (
        ...
        country_exposure_df,
        mktcap_exposure_df,
    ) = Exposures.filter_exposure_calc(filters_dict, position)
    # Excel equivalent ["ExpReport"]
    (
        ...
        filters_dict, position, factor_betas, factor_prices
    )
       
    rw.write_ExpReport("ExpReport", strat_exposure_df.reset_index(), sector_exposure_df.reset_index(),industry_exposure_df.reset_index(), country_exposure_df.reset_index(), mktcap_exposure_df.reset_index(),
                       strat_beta_adj_exposure_df.reset_index(),sector_beta_adj_exposure_df.reset_index(), industry_beta_adj_exposure_df.reset_index(), country_beta_adj_exposure_df.reset_index(),mktcap_beta_adj_exposure_df.reset_index())
    ...
    if saved_stress:     
        ...
        options_premium_calc = Exposures.filter_options_premium(position, factor_prices)
        # Excel equivalent ["Options&Stress"; "Greek Sensitivity" tbl]
        greek_sensitivities_calc = Exposures.greek_sensitivities(position, factor_prices)
    
        options_delta_adj_exposure_calc.to_pickle('options_delta_adj_exposure_calc.pkl')
        options_delta1_exposure_calc.to_pickle('options_delta1_exposure_calc.pkl')
        greek_sensitivities_calc.to_pickle('greek_sensitivities_calc.pkl')
        options_premium_calc.to_pickle('options_premium_calc.pkl')
        
    
    rw.write_optionstress("Options&Stress", stress_test_beta_price_vol_results_df.reset_index(), stress_test_price_vol_results_df.reset_index(), 
                          stress_test_price_vol_exposure_results_df.reset_index(),options_delta_adj_exposure_calc.reset_index(),
                          options_delta1_exposure_calc.reset_index(),greek_sensitivities_calc.reset_index(),options_premium_calc.reset_index())
    rw.close_writer()
    ...
    macro_factor_decomp_df, sector_factor_decomp_df = Exposures.factor_decomp_filtered(
        position, factor_betas, factor_prices, factor, matrix_cov
    )
    ...
    ) = Exposures.factor_decomp_by_factor_position(position, factor_betas, factor)
       
    rw.write_FactorExp("FactorExposures", macro_factor_decomp_df.reset_index(), sector_factor_decomp_df.reset_index(),risk_factor_exposure_top_N_list,risk_factor_exposure_bottom_N_list)
    

    ################### Excel equivalents ["FactorHeatMap"]
    factor_heat_map = Exposures.factor_heat_map(position, factor_betas, factor)
   
    rw.write_heatmap("FactorHeatMap", factor_heat_map.reset_index())
    ...
    (
        ...
        position, factor_betas, matrix_cov, position_returns, factor
    )
    
    rw.write_positionbd("PositionsBreakdown", position_breakdown.reset_index())
    rw.write_positionsummary("PositionsSummary", position_summary.reset_index())
   
    rw.close_writer()

    ################### 1.e. pnl estimation
    AUM_clean = pnl_stats.NAV_clean(AUM)
    ...
    gross_mkt_value_pct = (
        abs(position["MarketValue"]).sum() / position_agg_exposure["MarketValue"].sum()
    )
    net_mkt_value_pct = (
        position["MarketValue"].sum() / position_agg_exposure["MarketValue"].sum()
    )
    ...
    sector_exposure_df.to_excel(writer, sheet_name="Dashboard", startcol=1,
    ...
    macro_factor_decomp_df.to_excel(
        writer, sheet_name="Dashboard", startcol=1, startrow = 16+len(
            fund_exp_pct_dashboard)+len(sector_exposure_df)+(2*2)
    )
    # Excel equivalents ["Dashboard; "Sector Sensitivity" tbl]
    sector_factor_decomp_df.to_excel(
        writer, sheet_name="Dashboard", startcol=1, startrow = 16+len(
            fund_exp_pct_dashboard)+len(sector_exposure_df)+len(
            macro_factor_decomp_df) + (3*2)
    )
    # Excel equivalent ["Dashboard"; "Greek Sensitivity" tbl]
    greek_sensitivities_calc.to_excel(
        writer, sheet_name="Dashboard", startcol=1, startrow = 16+len(
            fund_exp_pct_dashboard)+len(sector_exposure_df)+len(
            macro_factor_decomp_df) + len(sector_factor_decomp_df) + (4*2)
    )
    # Excel equivalent ["Dashboard"; "Option Premium" tbl]
    options_premium_calc.to_excel(
        writer, sheet_name="Dashboard", startcol=9, startrow = 16+len(
            fund_exp_pct_dashboard)+len(sector_exposure_df)+len(
            macro_factor_decomp_df) + len(sector_factor_decomp_df) + (4*2)
    )
    ...
    factor_heat_map.to_excel(writer, sheet_name="FactorHeatMap", startcol=0, startrow=0)
    # Excel equivalent ["FactorExposures"; "Macro Factor Sensitivity" tbl & "Sector
    # Sensitivities" tbl]
    macro_factor_decomp_df.to_excel(
    ...
    startrow=(4 + len(macro_factor_decomp_df) + len(sector_factor_decomp_df) + (2*3))
    ...
    # Excel equivalent ["ExpReport"]
    strat_exposure_df.to_excel(writer, sheet_name="ExpReport", startcol=1, startrow=4)
    ...
    industry_exposure_df.to_excel(
        ...
        sheet_name="ExpReport",
        startcol=1,
        startrow=4 + len(strat_exposure_df) + len(sector_exposure_df) + (2 * 21),
    )
    ...
    VaR_Bottom10.to_excel(writer, sheet_name="VaRReport", startcol=5, startrow=4)
