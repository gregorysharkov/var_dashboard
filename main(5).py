...
if __name__ == "__main__":
    ...
    gross_mkt_value_pct = abs(position["MarketValue"]).sum() / firm_NAV.values[0]
    ...
    fund_exp_usd_dashboard = pd.DataFrame(
        {
            ...
            "Short": [
                ...
                sector_beta_adj_exposure_df["Short"].sum() * firm_NAV.values[0],
                short_mkt_value,
            ],
            "Gross": [
                sector_exposure_df["Gross"].sum() * firm_NAV.values[0],
                sector_beta_adj_exposure_df["Gross"].sum() * firm_NAV.values[0],
                gross_mkt_value,
            ],
            "Net": [
    ...
    macro_factor_decomp_df.to_excel(
        ...
        sheet_name="Dashboard",
        startcol=1,
        startrow=16 + len(fund_exp_pct_dashboard) + len(sector_exposure_df) + (2 * 2),
    )
